...
import com.example.techquiz.databinding.ActivitySpinWheelBinding;
import com.example.techquiz.model.LuckyItem;
import com.example.techquiz.spin.LuckyWheelView;
import com.example.techquiz.databinding.ActivitySpinWheelBinding;
...
ActivitySpinWheelBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.wheel.setData(data);//wheel id of wheelview in spinwheel layout
        binding.wheel.setRound(5);
        binding.spinbtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Random random=new Random();
                int rand=random.nextInt(8);
                binding.wheel.startLuckyWheelWithTargetIndex(rand);
            }
        });
        binding.wheel.setLuckyRoundItemSelectedListener(new LuckyWheelView.LuckyRoundItemSelectedListener() {
            @Override
    ...
    void updateCash(int index){
        ...
        database.collection("user").document(FirebaseAuth.getInstance().getUid())
            ...
            public void onSuccess(Void unused) {
                Toast.makeText(SpinWheel.this, "coins successfully added", Toast.LENGTH_SHORT).show();
                finish();
            }
        });
